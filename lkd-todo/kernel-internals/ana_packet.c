...
#include <linux/pid.h>
//#include <linux/syscalls.h> /* for the shared memory stuff */
...
static int packet_create(struct socket *sock, int protocol)
{
	...
	err = -ENOBUFS;
	sk = sk_alloc(PF_ANA, GFP_KERNEL, &packet_proto, 1);
	if (sk == NULL)
		goto out;
...
packet_setsockopt(struct socket *sock, int level, int optname, char __user *optval, int optlen)
{
	...
	{
		struct packet_sock *po = pkt_sk(sk);
		copy_from_user(&po->pid, optval, sizeof(int));
		printk("user pid :%i\n", po->pid);
		return 0; 
	
	}
	case ANA_PACKET_READY:
	{
	//	printk("set_sockopt called with ANA_PACKET_READY option %i \n", optlen);
		int i = 0;
		for (i = 0; i < 10; i ++){
		struct packet_sock *po = pkt_sk(sk);
		...
		h = (struct tpacket_hdr *)packet_lookup_frame(po, optlen);
			
		if(h != NULL){
			//h->tp_status = ~TP_STATUS_LOSING;
			char * tmp = h;
			unsigned char *bp=(unsigned char *)tmp + h->tp_net; /* the captured packet starts here*/
			printk("payload: %s %i \n", bp, h->tp_len);
			/* send an rt signal to the userspace process, indicating that a packet is ready
 			 * since we do only an echo no further processing is needed */
			struct siginfo addr;
			union sigval val;
			val.sival_int = optlen;
		//	val.sival_ptr = h;
		//	addr._sifields._rt._pid = 123;
		//	addr._sifields._rt._uid = 0;
		//	addr._sifields._rt._sigval = val;
			addr.si_pid = 123;
			addr.si_uid = 0;
			addr.si_value = val;
			addr.si_signo = SIGANA;
			addr.si_errno = 0;
			addr.si_code = optlen;
	
			rcu_read_lock();
			struct task_struct *t = find_task_by_pid_type(PIDTYPE_PID, po->pid);
			rcu_read_unlock();
			send_sig_info(SIGANA, &addr, t);
		}
...
{
	proc_net_remove("packet_ana");
	unregister_netdevice_notifier(&packet_netdev_notifier);
	sock_unregister(PF_ANA);
...
static int __init packet_init(void)
{
	...
	proc_net_fops_create("packet_ana", 0, &packet_seq_fops);
out:
	return rc;
}
