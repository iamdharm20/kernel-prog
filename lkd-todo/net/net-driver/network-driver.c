...
/* The function snull_rx is thus called after the hardware has received the packet */
static void nic_rx(struct net_device *netdev, int len, unsigned char *buf)
{
    struct sk_buff *skb;
    ...
    skb->dev = netdev;	/* define the device that got the packet. */
   
    /* Initialize the protocol field so that the packet is to be passed to an appropriate network layer protocol.*/
    ...
    netif_rx(skb); /* give it to the network layer */
}
...
* it will kick off the device interrupts and start the network transmit queue. After this function any network packets 
* directed to the hardware will start to arrive. Any incoming packets will also be sent into the system network interface subsystem. 
*/
static int nic_open(struct net_device *netdev)
{
    ...
    /* Now we are ready to accept the transmit request from queauing layer of the networking. */
...
* it will turn off the device interrupts and stop the network transmit queue.
*/
static int nic_close(struct net_device *netdev)
{
    struct nic_priv *priv = netdev_priv(netdev);
    netif_info(priv, ifdown, netdev, "%s(#%d), priv:%p\n", __func__, __LINE__, priv);
    /* The stop method should also stop the interface’s transmit queue*/
    netif_stop_queue(netdev);
    return 0;
}

/* Whenever the kernel needs to transmit a data packet, 
it calls the hard_start_transm it method to put the data on an outgoing queue*/
...
static netdev_tx_t nic_start_xmit(struct sk_buff *skb, struct net_device *netdev)
{
    struct nic_priv *priv = netdev_priv(netdev);
...
/* This function is called to fill up an eth header, since arp is not available on the interface */
...
/* ioctl commands */
int nic_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
    pr_info("ioctl\n");
    return 0;
}
...
static const struct net_device_ops nic_netdev_ops = {
    /* Kernel calls ndo_open() and ndo_validate_addr() when you bring up the NIC */
    .ndo_open               = nic_open,
    ...
    /* Kernel calls ndo_start_xmit() when it wants to transmit a packet. */
    .ndo_start_xmit         = nic_start_xmit,

    /* ndo_change_mtu() is called, when you change MTU */
    .ndo_change_mtu         = nic_change_mtu,

    /* ndo_set_mac_address() is called, when you change the MAC addr */
...
static struct net_device* nic_alloc_netdev(void)
{
    ...
    /* This will set up the ethernet name of the device (eth%d). The number (%d) will be assigned automatically by the kernel*/
...
static int __init nic_init(void)
{
    ...
    if (ret) {
        pr_err("%s(#%d): reg net driver failed. ret:%d", __func__, __LINE__, ret);
        goto reg1_failed;
    }
...
MODULE_DESCRIPTION("Network loop back driver");
MODULE_LICENSE("GPL");
