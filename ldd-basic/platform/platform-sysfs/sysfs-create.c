...
static void _power_on(struct my_device_platform_data *sd, struct platform_device *pdev)
{
  int result;
...
static void _power_off(struct my_device_platform_data *sd, struct platform_device *pdev)
{
   printk(KERN_INFO "custom device is powered off");
   device_remove_file(&pdev->dev, &dev_attr_value);
}

static void _reset(struct my_device_platform_data *sd)
{
...
static void _release(struct device *dev)
{
...
static int __init _platform_driver_init(void)
{
...
static void __exit _platform_driver_exit(void)
{
