...
static irqreturn_t _irq_handler(unsigned irq, void *dev_id)
{
...
static int __init _gpio_access_init(void)
{
   ...
   result = request_irq(irq_no, (irq_handler_t)_irq_handler,IRQF_TRIGGER_HIGH,
                        //IRQF_TRIGGER_MASK,
                        "platform-gpio-device", NULL);
   if (result < 0)
     {
